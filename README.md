...
Приложение предназначено для исследования собственных сетей и авторизованного тестирования. Убедитесь, что у вас есть письменное разрешение на перехват, деаутентификацию и подбор, и запускайте программу только с правами `root`/`sudo` там, где это допускается. REST API по умолчанию слушает только `127.0.0.1`; при выходе в сеть используйте `--api-token`/`--api-token-file` — без токена не открывайте его в недоверенные сети. Автор и сопровождающие не несут ответственности за незаконное использование.

О раскрытии уязвимостей — [SECURITY.md](SECURITY.md). О разработке и контрибьютинге — [CONTRIBUTING.md](CONTRIBUTING.md).
